...
module.exports = HandleMsg = async (pakforlay, message) => {
	...
	// Filter Banned People
            ...
            if (isOwnerBot) {
                await pakforlay.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await pakforlay.sendText(chekgrup.id, `Hai minna~, Im Lucas.Shop BOT. To find out the commands on this bot type ${prefix}menu`)
                      })
            } else {
        ...
        case 'insta':
            ...
            await pakforlay.sendFileFromUrl(from, instag, 'Sukses mengunduh Foto/Video Instagram Menggunakan Bot WhatsApp Lucas.Shop', '', id)
            .catch(() => {
                pakforlay.reply(from, 'Ada yang Error!', id)
            })
            break
        ...
        case 'ytmus':
            ...
            await pakforlay.sendFileFromUrl(from, ytmus, `Sukses mengunduh MP3 Youtube Menggunakan Bot WhatsApp Lucas.Shop`, id)
            .catch(() => {
                pakforlay.reply(from, 'Ada yang Error!', id)
            })
            break
		case 'ytvid':
            ...
            await pakforlay.sendFileFromUrl(from, youtube, `Sukses mengunduh Video Youtube Menggunakan Bot WhatsApp Lucas.Shop`, id)
            .catch(() => {
	...
	/*	case 'simisimi':
		...
		//intruksi Lucas
		case 'sistemberber':
		       pakforlay.reply(from, '  1.Pembeli Kirim Dana Ke Admin + Fee (Pajak Untuk Admin)\n\t2.dana masuk, dan dikirim ke penjual setelah penjual kirim data ke pembeli\n\t3.penjual kirim data ke pembeli untuk di amankan sendiri\n\t*AND DONE*\n\t*PENULIS BY LUCAS.SHOP*', id)
			   break
			   
		case 'kasihpaham':
		      pakforlay.reply(from, '*Jangan Lupa Gunakan Jasa Admin Lur, Sekecil apapun nominalnya lur👌🏻, Karena dengan menggunakan jasa admin Resiko penipuannya Kecil Lur🙏🏻. jangan karena akun bagus kamu Terkecoh ya lur🙏🏻, Be A Smart Buyer Lur👌🏻🙏🏻*', id)
			   break
			   
		case 'listripper':
		    await pakforlay.sendText(from, menuId.textRipper())
            break
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) pakforlay.sendText(idk, `〘 *Lucas.Shop  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) pakforlay.sendText(idk, `〘 *Lucas.Shop  B C* 〙\n\n${msg}`)
            }
