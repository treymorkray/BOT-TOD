...
const start = (pakforlay = new Client()) => {
    ...
    console.log(color(figlet.textSync('Lucas.Shop BOT', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('Lucas.Shop', 'yellow'))
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await pakforlay.simulateTyping(chat.id, true).then(async () => {
          await pakforlay.sendText(chat.id, `Hai minna~, Im Lucas.Shop BOT. To find out the commands on this bot type ${prefix}menu`)
        })
	    }
	}
    })
    ...
    pakforlay.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host) {
            await pakforlay.sendTextWithMentions(event.chat, `Hello, Selamat Datang Di Grub @${event.who.replace('@c.us', '')} \n\nJangan Lupa Senyum Hari Ini✨`)
        }
        // kondisi ketika seseorang dikick/keluar dari group
        if (event.action === 'remove' && event.who !== host) {
            await pakforlay.sendTextWithMentions(event.chat, `Selamat Jalan @${event.who.replace('@c.us', '')}, Semoga Tenang Di Alam Sana✨`)
        }
    })
